...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    alternates: { canonical: `/shop/rings/${data.id}`, languages: { en: `/en/shop/rings/${data.id}`, el: `/shop/rings/${data.id}` } },
  };
}

export default async function RingPage({ params }) {
  const resolvedParams = await params;
  const product = await fetchProduct(resolvedParams.id);
  return <IndividualProductPage params={resolvedParams} category="rings" initialProduct={product} />;
}
